...
public class Equal extends DALOperator {
    ...
    public Data calculateData(DALExpression expression, RuntimeContextBuilder.DALRuntimeContext context) {
        if (expression.right() instanceof TableNode || expression.right() instanceof TransposedTableNode
                || expression.right() instanceof ListScopeNode || expression.right() instanceof ObjectScopeNode
                || expression.right() instanceof RegexNode || expression.right() instanceof WildcardNode)
